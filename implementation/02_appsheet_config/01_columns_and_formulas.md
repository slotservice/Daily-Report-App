...
| **Related Tasks (in progress)** *(virtual)* | List | H | `SELECT(Tasks[TaskID], AND([ProjectID] = [_THISROW].[ProjectID], [Status] <> "Completed", [StartDate] <= [_THISROW].[ReportDate]))` | **Display name: `Tasks in Progress`** (per Evan 2026-05-14 — exact wording, lower-case `i` in "in"). **`Show?` = `CONTEXT("ViewType") = "Detail"`** (set 2026-05-13) — inline reference list on `Today's Report` detail view, hidden in the create form. |
...
| **Related Tasks (completed today)** *(virtual)* | List | H | `SELECT(Tasks[TaskID], AND([ProjectID] = [_THISROW].[ProjectID], [Status] = "Completed", [CompletedDate] = [_THISROW].[ReportDate]))` | **Display name: `Tasks Completed Today`** (per Evan 2026-05-14). **`Show?` = `CONTEXT("ViewType") = "Detail"`** (set 2026-05-13). Per Evan 2026-05-11 item 5 — completed tasks appear here in their own section. |
...
| **Related ReportTrades (still on site)** *(virtual)* | List | H | `SELECT(ReportTrades[ReportTradeID], AND([ProjectID] = [_THISROW].[ProjectID], [Status] = "On Site"))` | **Display name: `Trades on Site Today`** (per Evan 2026-05-14 — exact wording. Previously the raw column name was showing in the Detail view because the Display Name override was never set in the live build; Evan flagged this as item 2 of his 2026-05-14 batch). **Per Evan 2026-05-10 (item 13): trades carry forward like tasks in progress.** **`Show?` = `CONTEXT("ViewType") = "Detail"`** (set 2026-05-13). This is the list the super sees on today's report under "Trades on Site Today". When a trade leaves site, the super taps the inline "Mark Off Site" action on Action 8 (`03_actions.md`), which flips `ReportTrades.Status` to `"Off Site"` and the row drops out of this virtual column. New trades are added via the **`+ Add Trade` action (Action 11 in `03_actions.md`, added 2026-05-14)** which calls `LINKTOFORM("ReportTrades_Form", "ReportID", [ReportID], "ProjectID", [ProjectID], "Status", "On Site", "OriginReportID", [ReportID])`. |
...
| **Related Equipment (still on site)** *(virtual)* | List | H | `SELECT(Equipment[EquipmentID], AND([ProjectID] = [_THISROW].[ProjectID], [Status] = "On Site"))` | **Display name: `Equipment on Site Today`** (per Evan 2026-05-14 — exact wording, lower-case `o` in "on". Note this differs from earlier spec drafts that used capital `O`; Evan's 2026-05-14 wording wins). **Per Evan 2026-05-10 (item 12): equipment carries forward.** **`Show?` = `CONTEXT("ViewType") = "Detail"`** (set 2026-05-13). Same pattern as trades above — Action 9 in `03_actions.md` flips `Equipment.Status` to `"Off Site"` when the super taps the inline "Mark Off Site" check. |
| **Related Rentals** *(virtual)* | List | H | `REF_ROWS("Rentals", "ReportID")` | Per-report row history (audit). **`Show?` = `FALSE`** (set 2026-05-14) — this is the raw audit list; the user-facing list is the `(still on site)` variant below. The previous Display Name `Rentals on Site Today` has been **moved to the new `[Related Rentals (still on site)]` virtual column** as part of the 2026-05-14 carry-forward rollout. Internal name kept the same so the auto-generated PDF binding does not break for any existing report; the binding is now redirected to the new virtual column instead — see `GoogleDoc_template_setup.md`. |
| **Related Rentals (still on site)** *(virtual)* | List | H | `SELECT(Rentals[RentalID], AND([ProjectID] = [_THISROW].[ProjectID], [Status] = "On Site"))` | **NEW 2026-05-14** — per Evan 2026-05-14 (item 7, option a): rentals carry forward like equipment + trades. **Display name: `Rentals on Site Today`** (moved from the old `[Related Rentals]` column). **`Show?` = `CONTEXT("ViewType") = "Detail"`**. This is the list the super sees on today's report under "Rentals on Site Today". When a rental is returned, the super taps the inline `Mark Rental Returned` action (Action 12 in `03_actions.md`), which flips `Rentals.Status` to `"Returned"` and the row drops out of this virtual column. New rentals are added via the inline `+ Add Rental` action (Action 13) which calls `LINKTOFORM("Rentals_Form", "ReportID", [ReportID], "ProjectID", [ProjectID], "Status", "On Site", "OriginReportID", [ReportID])`. |
...
| Description | Text | L |  | **Display name: `Task - Trade`** (per Evan 2026-05-14 — applies to the Tasks form input label AND to the column header on every Tasks inline list across the app: Started Today, In Progress, Completed Today). Required. Internal column name stays `Description`; PDF template repeat tokens that reference `<<[Description]>>` are unaffected. |
...
Per Evan 2026-05-14 (item 7) the rentals list **carries forward** day to day, mirroring the Equipment / ReportTrades pattern from 2026-05-10 (items 12 + 13). Same additive change: new `ProjectID`, `Status`, `ReturnedDate`, `OriginReportID` columns. Existing columns unchanged.

A rental added once stays visible in the project's `Rentals on Site Today` section every day until the super taps the inline `Mark Rental Returned` check (Action 12 in `03_actions.md`), which flips `Status` to `"Returned"` and stamps `ReturnedDate`. The row then drops out of the `[Related Rentals (still on site)]` carry-forward virtual column on `DailyReports` (§3 above).

| Column | Type | K/L/H | Initial value | Notes |
|---|---|---|---|---|
| RentalID | Text | K | `"REN-" & TEXT(COUNT(Rentals[RentalID])+1, "0000")` |  |
| ReportID | Ref → DailyReports | H |  | Required at create. First-report pointer. Hidden in form (prefilled from parent). |
| ProjectID | Ref → Projects | H | `LOOKUP([ReportID], "DailyReports", "ReportID", "ProjectID")` | **NEW 2026-05-14.** Required. Auto-derived from ReportID at create — never edited by the user. Drives the project-wide carry-forward virtual column on `DailyReports`. Hidden in the form. |
| Description | Text | L |  | Required |
| PONumber | Text |  |  | Required (per spec: "Rentals (PO Number)") |
| Supplier | Text |  |  |  |
| DailyRate | Decimal |  |  |  |
| Notes | LongText |  |  |  |
| Status | Enum |  | `"On Site"` | **NEW 2026-05-14.** Display name: `Status`. Values: `On Site, Returned`. Base type Text. Required. Drives whether the row appears in tomorrow's `Rentals on Site Today` carry-forward list. Editable_If: super of the project, or Admin. Toggle is via the inline `Mark Rental Returned` action (Action 12) in `03_actions.md`, not by the user editing this column directly — but direct edit is allowed for cleanup. |
| ReturnedDate | Date | H |  | **NEW 2026-05-14.** Set by `Mark Rental Returned` (Action 12). Editable_If: `[Status] = "Returned"`. Hidden in form. |
| OriginReportID | Ref → DailyReports | H |  | **NEW 2026-05-14.** Set by `+ Add Rental` action — the report on which this rental was first added. Editable_If: `FALSE`. Mirrors `Equipment.OriginReportID` and `ReportTrades.OriginReportID`. |
