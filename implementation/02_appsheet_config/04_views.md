...
- **Layout sections (in this exact order — pin in the AppSheet detail-view editor; do not let AppSheet auto-group reorder them):**
  ...
  5. **Tasks in Progress** — inline reference list of `[Related Tasks (in progress)]` (read-only carry-forward), with inline **Mark Task Completed** action (Action 3) shown as a check-square icon per row. **Section heading renamed 2026-05-14 (item 3)** — exact wording: `Tasks in Progress` (was `Tasks Still in Progress`). Set this as the Display Name on the `[Related Tasks (in progress)]` virtual column on `DailyReports`.
  6. **Tasks Completed Today** — inline reference list of `[Related Tasks (completed today)]`. Display Name confirmed by Evan 2026-05-14 (item 5). **Section must always render**, even when empty (use AppSheet's "Show empty group" toggle on the inline ref-list). Per Evan 2026-05-11 item 9: tasks marked completed must appear here within one sync cycle. The virtual column filter is `[CompletedDate] = [_THISROW].[ReportDate]` — note this implies a task completed on a day OTHER than the report's date (e.g., a backdated edit) won't appear; this is intentional, day-of-completion accuracy.
  7. **Equipment on Site Today** — inline reference list of `[Related Equipment (still on site)]` (the carry-forward virtual column — per Evan 2026-05-10 item 12). **Section heading renamed 2026-05-14 (item 4)** — exact wording: `Equipment on Site Today` (lower-case `o`; was `Equipment On Site Today`). Set this as the Display Name on the `[Related Equipment (still on site)]` virtual column. Inline `+ Add Equipment` action calls `LINKTOFORM("Equipment_Form", "ReportID", [ReportID], "ProjectID", [ProjectID], "Status", "On Site", "OriginReportID", [ReportID])`. Inline **Mark Equipment Off Site** action (Action 9) per row.
  8. **Rentals on Site Today** — inline reference list of `[Related Rentals (still on site)]` (the carry-forward virtual column added 2026-05-14 per Evan item 7, option a). **The old `[Related Rentals]` REF_ROWS column is hidden (Show? = FALSE)** and the Display Name `Rentals on Site Today` has moved to the new virtual column. Inline `+ Add Rental` action (Action 13 in `03_actions.md`) calls `LINKTOFORM("Rentals_Form", "ReportID", [ReportID], "ProjectID", [ProjectID], "Status", "On Site", "OriginReportID", [ReportID])`. Inline **Mark Rental Returned** action (Action 12) shown as a check-square icon per row — same pattern as Mark Equipment Off Site / Mark Trade Off Site.
